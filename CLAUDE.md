...
- **IMPORTANT**: Main Claude MUST save the PRD to `/home/swq/Documents/github/jargav3/docs/features/[feature-name]-prd.md`
...
- **IMPORTANT**: Main Claude MUST save the implementation plan to `/home/swq/Documents/github/jargav3/docs/features/[feature-name]-implementation-plan.md`
- **TodoList.md file** in project root with all checkboxes organized by phase

**After Planning Phase 2, Main Claude MUST:**
...
**Main Claude Action After Phase 2:**

Before proceeding to Phase 3, Main Claude MUST:
...
**Main Claude Action After Phase 4:**

Before proceeding to QA phases, Main Claude MUST:
...
2. **Main Claude** reads TodoList.md between phases to track progress
...
4. **Main Claude Pre-commit Checkpoints** (after Phase 2 and Phase 4):
...
5. **QA agents** (test-validator, code-reviewer, doc-sync):
   - Read their QA phase section
   - Check off validation items as they complete them
   - Update their phase status when done
6. **Main Claude** coordinates handoffs between agents using TodoList.md status
...
- Main Claude can see overall feature progress at a glance
...
#### QA Phase 3: Documentation Sync (Use `doc-sync` subagent)
...
QA Phase 2: [code-reviewer] → Ensures architectural integrity
    ↓
QA Phase 3: [doc-sync] → Extracts patterns and updates docs
    ↓
...
Quality Assurance Phases:
  ...
  QA Phase 3: doc-sync → Document pattern
```
...
(All agents read TodoList.md and check off items as they complete them)
  ...
  Implementation Phase 1: Backend domain + application layers
    [Main Claude updates TodoList.md Phase 1 status: ⏸ → ⏳]
  ...
  Implementation Phase 2: Backend infrastructure + interface layers
    [Main Claude updates TodoList.md Phase 2 status: ⏸ → ⏳]
    [Delegates to phoenix-tdd with Phase 2 scope]
    Output: "Phase 2 complete. All checkboxes ticked. Status updated to ✓"

  Main Claude Pre-commit Checkpoint (After Phase 2):
    [Main Claude runs: mix precommit]
  ...
  Implementation Phase 3: Frontend domain + application layers
    [Main Claude updates TodoList.md Phase 3 status: ⏸ → ⏳]
  ...
  Implementation Phase 4: Frontend infrastructure + presentation layers
    [Main Claude updates TodoList.md Phase 4 status: ⏸ → ⏳]
    [Delegates to typescript-tdd with Phase 4 scope]
    Output: "Phase 4 complete. All checkboxes ticked. Status updated to ✓"

  Main Claude Pre-commit Checkpoint (After Phase 4):
    [Main Claude runs: mix precommit]
    [Main Claude runs: npm test]
...
(All QA agents also use TodoList.md for their checklists)
  ...
  QA Phase 1: Verify TDD process across all layers...
    [Main Claude updates TodoList.md QA Phase 1 status: ⏸ → ⏳]
  ...
  QA Phase 2: Check architectural compliance...
    [Main Claude updates TodoList.md QA Phase 2 status: ⏸ → ⏳]
    [Delegates to code-reviewer]
    Output: "Code review complete. TodoList.md updated to ✓"

  QA Phase 3: Update documentation with new patterns...
    [Main Claude updates TodoList.md QA Phase 3 status: ⏸ → ⏳]
    [Delegates to doc-sync]
    Output: "Documentation sync complete. TodoList.md updated to ✓"
...
🤖 **Subagent Details:**
- `.claude/agents/prd.md` - Requirements gathering and PRD creation
- `.claude/agents/architect.md` - Feature planning process
- `.claude/agents/phoenix-tdd.md` - Phoenix backend and LiveView TDD implementation
- `.claude/agents/typescript-tdd.md` - TypeScript TDD implementation
