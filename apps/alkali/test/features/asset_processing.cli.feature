...
Feature: Asset Pipeline with Minification and Fingerprinting
  ...
  Background:
    ...
    When I run "rm -rf _site static"
    When I run "mkdir -p static/css static/js static/images _site"
  ...
  Scenario: Minify and fingerprint CSS
    ...
    When I run "wc -c < static/css/app.css"
    Then the command should succeed
  ...
  Scenario: Update asset references in HTML
    ...
    When I run "cat _site/index.html"
    Then the command should succeed
  ...
  Scenario: Copy static assets without processing
    ...
    When I run "printf 'PNG_PLACEHOLDER' > static/images/logo.png"
    Then the command should succeed
    # Run the build
    When I run "mix alkali.build"
    Then the command should succeed
    # The file should be copied to the output directory
    When I run "test -f _site/images/logo.png && echo exists"
    Then the command should succeed
    And stdout should contain "exists"
    # The file should be identical to the original
    When I run "diff static/images/logo.png _site/images/logo.png && echo identical"
    Then the command should succeed
