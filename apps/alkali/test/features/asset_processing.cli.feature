...
Feature: Asset Pipeline with Minification and Fingerprinting
  ...
  Scenario: Update asset references in HTML
    ...
    When I run "printf '%b' '<html><head><link rel=\"stylesheet\" href=\"/css/app.css\"></head><body><%= @content %></body></html>\n' > ${site}/layouts/default.html.heex"
    Then the command should succeed
    # Create content that will use the default layout
    When I run "printf '%b' '---\ntitle: Home\n---\nWelcome\n' > ${site}/content/index.md"
    Then the command should succeed
    # Create the CSS file that will be fingerprinted
    When I run "printf '%b' 'body { margin: 0; }\n' > ${site}/static/css/app.css"
    Then the command should succeed
    # Run the build
    When I run "mix alkali.build ${site}"
    Then the command should succeed
  ...
  Scenario: Copy static assets without processing
    ...
    When I run "printf '%b' 'PNG_PLACEHOLDER' > ${site}/static/images/logo.png"
    Then the command should succeed
    # Run the build
    When I run "mix alkali.build ${site}"
    Then the command should succeed
