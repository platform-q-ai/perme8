defmodule KnowledgeMcp.Application.UseCases.CreateKnowledgeEntryTest do
  use ExUnit.Case, async: true

  import Mox

  alias KnowledgeMcp.Application.UseCases.CreateKnowledgeEntry
  ...
  defp setup_bootstrap_mock do
    # Bootstrap always succeeds (schema already exists)
    ErmGatewayMock
  ...
  describe "execute/3" do
    ...
    test "calls BootstrapKnowledgeSchema first" do
      # We can verify that get_schema is called (part of bootstrap)
      ErmGatewayMock
      |> expect(:get_schema, fn _ws_id -> {:ok, schema_definition_with_knowledge()} end)
    ...
    test "creates ERM entity with type KnowledgeEntry and JSON-encoded list properties" do
      ...
      |> expect(:create_entity, fn _ws_id, attrs ->
        assert attrs.type == "KnowledgeEntry"
        # List fields should be JSON-encoded
