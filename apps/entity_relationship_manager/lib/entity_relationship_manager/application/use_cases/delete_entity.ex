defmodule EntityRelationshipManager.Application.UseCases.DeleteEntity do
  ...
  """
  def execute(workspace_id, entity_id, actor_id, opts \\ []) do
    ...
    with :ok <- InputSanitizationPolicy.validate_uuid(entity_id),
      ...
      emit_entity_deleted_event(deleted_entity, workspace_id, actor_id, event_bus)
      {:ok, deleted_entity, edge_count}
    end
  end

  defp emit_entity_deleted_event(entity, workspace_id, actor_id, event_bus) do
    event =
      EntityDeleted.new(%{
