defmodule Mix.Tasks.StaticSite.PostTest do
  use ExUnit.Case
  ...
  describe "static_site.new.post" do
    test "creates post in specified site path" do
      ...
      output =
        capture_io(fn ->
          Mix.Tasks.StaticSite.New.Post.run(["My Post", site_path])
        end)
    ...
    test "creates post using --path option" do
      ...
      output =
        capture_io(fn ->
          Mix.Tasks.StaticSite.New.Post.run(["Option Post", "--path", site_path])
        end)
  ...
  describe "static_site.post" do
    test "creates post in specified site path" do
      ...
      output =
        capture_io(fn ->
          Mix.Tasks.StaticSite.Post.run(["Short Post", site_path])
        end)
    ...
    test "creates post using --path option" do
      ...
      output =
        capture_io(fn ->
          Mix.Tasks.StaticSite.Post.run(["Short Option", "--path", site_path])
        end)
