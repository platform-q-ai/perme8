defmodule StaticSite.CommonSteps do
  @moduledoc """
        ...
        output =
          capture_io(fn ->
            File.cd!(work_dir, fn ->
              ...
              case task_name do
                "static_site.new" -> Mix.Tasks.StaticSite.New.run(args)
                "static_site.new.post" -> Mix.Tasks.StaticSite.New.Post.run(args)
                "static_site.build" -> Mix.Tasks.StaticSite.Build.run(args)
                "static_site.post" -> Mix.Tasks.StaticSite.Post.run(args)
                "static_site.clean" -> Mix.Tasks.StaticSite.Clean.run(args)
                _ -> :ok
              end
            end)
          end)
  ...
  defp setup_temp_dir do
    tmp_dir = System.tmp_dir!() <> "/static_site_test_#{:os.system_time(:millisecond)}"
    File.rm_rf!(tmp_dir)
  ...
  # Example: "mix static_site.new.post \"My Title\"" -> {"static_site.new.post", ["My Title"]}
