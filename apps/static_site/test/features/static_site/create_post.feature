Feature: Create New Blog Post
  ...
  Scenario: Create new post with title
    When I run "mix static_site.new.post 'Getting Started with Elixir'"
  ...
  Scenario: Create new post with title and site path
    When I run "mix static_site.new.post 'Another Post' test_site"
    Then the command should succeed
  ...
  Scenario: Create new post with --path option
    When I run "mix static_site.new.post 'Option Post' --path test_site"
    Then the command should succeed
  ...
  Scenario: Short task command with site path
    When I run "mix static_site.post 'Short Post' test_site"
    Then the command should succeed
