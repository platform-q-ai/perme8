defmodule StaticSite.Application.UseCases.BuildSite do
  @moduledoc """
  ...
  def execute(site_path, opts \\ []) do
    ...
    cache =
      if incremental do
        ...
        StaticSite.Infrastructure.BuildCache.load(site_path)
      else
        %{}
      end
    ...
    if incremental do
      ...
      updated_cache = StaticSite.Infrastructure.BuildCache.update_cache(cache, all_file_paths)

      StaticSite.Infrastructure.BuildCache.save(site_path, updated_cache)
    end
  ...
  defp filter_changed_pages(pages, cache, config, verbose) do
    ...
    alias StaticSite.Infrastructure.BuildCache
  ...
  defp generate_rss_feed(pages, config, file_writer, mkdir_fn, opts, verbose) do
    ...
    if not generate_rss do
      ...
      if is_nil(site_url) or site_url == "" do
        ...
        case StaticSite.Application.UseCases.GenerateRssFeed.execute(pages, rss_opts) do
          {:ok, xml} ->
  ...
  defp render_and_write_pages(pages, config, collections, mappings, opts, verbose) do
    alias StaticSite.Infrastructure.LayoutResolver
  ...
  defp render_paginated_collection(
    ...
    alias StaticSite.Application.Helpers.Paginate
