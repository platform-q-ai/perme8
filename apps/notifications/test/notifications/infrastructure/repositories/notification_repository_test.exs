defmodule Notifications.Infrastructure.Repositories.NotificationRepositoryTest do
  ...
  describe "get/1" do
    ...
    test "returns nil when notification doesn't exist" do
      assert NotificationRepository.get(Ecto.UUID.generate()) == nil
    end
  end
  ...
  describe "get_by_user/2" do
    ...
    test "returns nil when notification doesn't exist" do
      ...
      assert NotificationRepository.get_by_user(Ecto.UUID.generate(), user.id) == nil
    end
  end
  ...
  describe "list_by_user/2" do
    test "returns all notifications for user ordered by most recent" do
      user = user_fixture()
      notification1 = notification_fixture(user.id)
      notification2 = notification_fixture(user.id)
      notification3 = notification_fixture(user.id)
    ...
    test "does not return notifications for other users" do
      user1 = user_fixture()
      user2 = user_fixture()
      _notification = notification_fixture(user1.id)
  ...
  describe "list_unread_by_user/1" do
    test "returns only unread notifications" do
      user = user_fixture()
      unread1 = notification_fixture(user.id)
      unread2 = notification_fixture(user.id)
      read_notification = notification_fixture(user.id)
  ...
  describe "mark_all_as_read/1" do
    ...
    test "only marks notifications for specified user" do
      user1 = user_fixture()
      user2 = user_fixture()
      _n1 = notification_fixture(user1.id)
      _n2 = notification_fixture(user2.id)
