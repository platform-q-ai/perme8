defmodule Agents.Infrastructure.Mcp.Tools.Pr.CloseToolTest do
  use Agents.DataCase, async: false

  import Mox

  alias Agents.Infrastructure.Mcp.Tools.Pr.CloseTool
  ...
  setup do
    ...
    {:ok, pr} =
      Pipeline.create_pull_request(
        %{
          source_branch: "feature/close-tool",
          target_branch: "main",
          title: "Close me",
          status: "open"
        },
  ...
  test "closes pull request", %{pr: pr} do
    frame =
      Frame.new(%{
        workspace_id: "ws-1",
        user_id: "user-1",
    ...
    assert {:reply, response, ^frame} = CloseTool.execute(%{"number" => pr.number}, frame)
