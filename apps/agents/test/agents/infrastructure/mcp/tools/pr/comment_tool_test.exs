defmodule Agents.Infrastructure.Mcp.Tools.Pr.CommentToolTest do
  use Agents.DataCase, async: false

  import Mox

  alias Agents.Infrastructure.Mcp.Tools.Pr.CommentTool
  ...
  setup do
    ...
    {:ok, pr} =
      Pipeline.create_pull_request(%{
        source_branch: "feature/comment-tool",
        target_branch: "main",
        title: "Comment me",
        status: "in_review"
      })
  ...
  test "adds comment to pull request", %{pr: pr} do
    frame =
      Frame.new(%{
        workspace_id: "ws-1",
        user_id: "user-1",
    ...
    assert {:reply, response, ^frame} =
             CommentTool.execute(%{"number" => pr.number, "body" => "nit: rename this"}, frame)
