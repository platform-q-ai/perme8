defmodule Agents.Infrastructure.Mcp.Tools.Ticket.RemoveSubIssueToolTest do
  ...
  describe "execute/2" do
    test "removes sub-issue link and returns success" do
      frame = build_frame()

      Agents.Mocks.GithubTicketClientMock
      |> expect(:remove_sub_issue, fn 1, 2, _opts ->
        {:ok, %{parent_number: 1, child_number: 2}}
      end)

      assert {:reply, response, ^frame} =
               RemoveSubIssueTool.execute(%{"parent_number" => 1, "child_number" => 2}, frame)
    ...
    test "returns generic error for api rejection" do
      ...
      Agents.Mocks.GithubTicketClientMock
      |> expect(:remove_sub_issue, fn 1, 999, _opts ->
    ...
    test "denies execution when scope is missing" do
      ...
      |> expect(:api_key_has_permission?, fn ^api_key, "mcp:ticket.remove_sub_issue" -> false end)
