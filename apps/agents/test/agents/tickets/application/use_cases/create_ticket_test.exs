defmodule Agents.Tickets.Application.UseCases.CreateTicketTest do
  ...
  describe "execute/2" do
    test "inserts a local ticket with pending_push sync state" do
      assert {:ok, ticket} = CreateTicket.execute("Fix the login bug", actor_id: @actor_id)
    ...
    test "splits first line as title and rest as body" do
      ...
      assert {:ok, ticket} = CreateTicket.execute(input, actor_id: @actor_id)
    ...
    test "single line input sets title and empty body" do
      assert {:ok, ticket} = CreateTicket.execute("Single line ticket", actor_id: @actor_id)
    ...
    test "returns error for empty body" do
      assert {:error, :body_required} = CreateTicket.execute("", actor_id: @actor_id)
    end
    ...
    test "returns error for nil body" do
      assert {:error, :body_required} = CreateTicket.execute(nil, actor_id: @actor_id)
    end

    test "assigns a negative temporary number" do
      assert {:ok, ticket} = CreateTicket.execute("Test ticket", actor_id: @actor_id)
    ...
    test "generates unique numbers for concurrent inserts" do
      results =
        1..5
        |> Enum.map(fn i ->
          CreateTicket.execute("Ticket #{i}", actor_id: @actor_id)
        end)
    ...
    test "assigns a position to the new ticket" do
      assert {:ok, ticket} = CreateTicket.execute("Positioned ticket", actor_id: @actor_id)
