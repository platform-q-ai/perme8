defmodule Agents.Tickets.Application.UseCases.RecordStageTransition do
  ...
  def execute(ticket_id, to_stage, opts \\ []) do
    ...
    with {:ok, ticket} <- fetch_ticket(ticket_repo, ticket_id),
         :ok <- TicketLifecyclePolicy.valid_transition?(ticket.lifecycle_stage, to_stage),
         {:ok, lifecycle_event} <-
         ...
         {:ok, updated_ticket} <- ticket_repo.update_lifecycle_stage(ticket_id, to_stage, now) do
      event_bus.emit(
      ...
      {:ok, %{ticket: updated_ticket, lifecycle_event: lifecycle_event}}
    end
  end
