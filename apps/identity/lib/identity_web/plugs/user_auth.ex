defmodule IdentityWeb.Plugs.UserAuth do
  @moduledoc """
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      IdentityWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end

    conn
    |> renew_session(nil)
    |> delete_resp_cookie(@remember_me_cookie)
    |> redirect(to: @logged_out_redirect_path)
  end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      IdentityWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule IdentityWeb.PageLive do
        use IdentityWeb, :live_view

        on_mount {IdentityWeb.Plugs.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{IdentityWeb.Plugs.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_sudo_mode, _params, session, socket) do
    ...
    if Identity.sudo_mode?(socket.assigns.current_scope.user, minutes: -10) do
      {:cont, socket}
    else
      socket =
        socket
