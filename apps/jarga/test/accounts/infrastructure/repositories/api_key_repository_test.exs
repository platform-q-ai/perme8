defmodule Jarga.Accounts.Infrastructure.Repositories.ApiKeyRepositoryTest do
  ...
  describe "insert/2" do
    test "insert creates new API key" do
      ...
      assert {:ok, schema} = ApiKeyRepository.insert(Identity.Repo, attrs)
      assert schema.id
  ...
  describe "update/2" do
    test "update modifies existing API key" do
      ...
      {:ok, api_key} =
        ApiKeyRepository.insert(Identity.Repo, %{
          name: "Original Name",
          hashed_token: "hashed_token_123",
          user_id: user.id
        })
      ...
      assert {:ok, updated} =
               ApiKeyRepository.update(Identity.Repo, api_key.id, %{name: "Updated Name"})
  ...
  describe "get_by_id/2" do
    test "get_by_id returns API key" do
      ...
      assert {:ok, found} = ApiKeyRepository.get_by_id(Identity.Repo, api_key.id)
    ...
    test "get_by_id returns :not_found for non-existent id" do
      result = ApiKeyRepository.get_by_id(Identity.Repo, Ecto.UUID.generate())
  ...
  describe "get_by_hashed_token/2" do
    test "get_by_hashed_token returns API key" do
      ...
      assert {:ok, found} =
               ApiKeyRepository.get_by_hashed_token(Identity.Repo, "hashed_token_123")

      assert found.hashed_token == "hashed_token_123"
    end

    test "get_by_hashed_token returns :not_found for non-existent token" do
      result = ApiKeyRepository.get_by_hashed_token(Identity.Repo, "non_existent_token")
  ...
  describe "list_by_user_id/2" do
    test "list_by_user_id returns all user's API keys" do
      ...
      {:ok, keys} = ApiKeyRepository.list_by_user_id(Identity.Repo, user.id)
