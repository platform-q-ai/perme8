defmodule Jarga.Documents.UseCases.SaveMessageTest do
  @moduledoc """
  ...
  alias Jarga.Documents.UseCases.SaveMessage
  alias Jarga.Repo
  ...
  describe "execute/1" do
    test "saves a user message" do
      ...
      assert {:ok, message} = SaveMessage.execute(%{
      ...
      persisted = Repo.get(Jarga.Documents.ChatMessage, message.id)
    ...
    test "returns error when chat_session_id is missing" do
      assert {:error, changeset} = SaveMessage.execute(%{
        role: "user",
        content: "Hello"
      })
    ...
    test "saves messages in order for a session" do
      ...
      {:ok, msg1} = SaveMessage.execute(%{
        chat_session_id: session.id,
        role: "user",
        content: "First message"
      })

      {:ok, msg2} = SaveMessage.execute(%{
        chat_session_id: session.id,
        role: "assistant",
        content: "Second message"
      })

      {:ok, msg3} = SaveMessage.execute(%{
        chat_session_id: session.id,
        role: "user",
      ...
      assert Enum.map(messages, & &1.content) == ["First message", "Second message", "Third message"]
    end
  end
end
