defmodule Jarga.Documents.UseCases.DeleteSessionTest do
  @moduledoc """
  ...
  alias Jarga.Documents.UseCases.DeleteSession
  alias Jarga.Repo
  ...
  describe "execute/2" do
    test "deletes a session and its messages" do
      ...
      assert Repo.get(Jarga.Documents.ChatMessage, msg1.id) == nil
      assert Repo.get(Jarga.Documents.ChatMessage, msg2.id) == nil
    end
    ...
    test "only deletes messages from the specified session" do
      ...
      assert Repo.get(Jarga.Documents.ChatSession, session1.id) == nil
      assert Repo.get(Jarga.Documents.ChatMessage, msg1.id) == nil

      # Session2 and its message should still exist
      assert Repo.get(Jarga.Documents.ChatSession, session2.id) != nil
      assert Repo.get(Jarga.Documents.ChatMessage, msg2.id) != nil
    end
  end
end
